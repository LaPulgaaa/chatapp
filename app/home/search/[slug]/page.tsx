...
export default function Explorer({ params }: { params: { slug: string } }) {
  ...
  return resultState.state === "hasValue" ? (
    <>
...
function DirectMessageSearchResults({
  dms,
}: {
  dms: DirectMessageSearchResult;
}) {
  const router = useRouter();
    ...
    <div>
      {session.status === "authenticated" &&
        dms.map((dm) => {
          ...
          const rendered_content = is_send_by_me
            ? `You: ${dm.content}`
            : dm.content;
          return (
            <div
            ...
            >
              <div className="w-full flex flex-col space-y-2">
                <div className="flex justify-between">
                  <Link
                    href={`/dm/${slug}`}
                    className="text-muted-foreground text-sm hover:text-black hover:dark:text-white"
                  >{`${slug}`}</Link>
        ...
        chats.map((chat) => {
          ...
          return (
            <div
              key={chat.id}
              onClick={() =>
                router.push(`/chat/${chat.chatId}/near/${chat.id}`)
              }
