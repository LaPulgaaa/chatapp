...
function DirectMessageSearchResults({query}:{query: string}){
  ...
  const matched_results = useMemo(() => {
    if(dmState.state !== "hasValue" || query.length <= 2)
        return [];
    ...
    return (
        <div className="flex justify-end items-center shadow z-10">
            <div className="relative w-1/2 space-y-2">
                <SearchBox search={search} setSearch={setSearch} setFocus={setFocus} isMouseOverResults={overResults}/>
                {
                    focus && search.length > 2 && <div 
