...
function is_same_recipient(new_recipient:Recipient | null , old_recipient: Recipient | null){
    if(new_recipient === null || old_recipient === null)
        return false;
    if(new_recipient.message_type === "DM" && old_recipient.message_type === "DM"){
        return new_recipient.conc_id === old_recipient.conc_id;
    }
    if(new_recipient.message_type === "CHAT" && old_recipient.message_type === "CHAT"){
...
function send_chat_typing_notification(operation: "start" | "stop", room_id: string, user_id: string){
    const typing_data = {
        type: "CHAT" as const,
        operation,
        room_id,
...
function send_dm_typing_notification(operation: "start" | "stop",conc_id: string,user_id: string){
    const typing_data = {
        type: "DM" as const,
        operation,
        conc_id,
...
function send_typing_notification_based_on_msg_type(operation: "start" | "stop",recipient: Recipient){
    if(recipient.message_type === "DM"){
        return send_dm_typing_notification(operation,recipient.conc_id,recipient.user_id)
    }
    else if(recipient.message_type === "CHAT"){
        return send_chat_typing_notification(operation,recipient.room_id,recipient.user_id);
    }
...
export function ComposeBox(
    ...
    function maybe_notify_server_start(recipient: Recipient,worker:TypingStatusWorker,typing_start_wait_period: number, typing_stop_wait_period: number){
        ...
        setTypingState({
            current_recipient: recipient,
            next_send_start_time: worker.get_current_time() + typing_start_wait_period,
    ...
    function start_or_extend_idle_timer(worker: TypingStatusWorker,typing_stop_wait_period: number){
    ...
    function update(
        ...
        if(typingState !== null){
            if(is_same_recipient(typingState.current_recipient,new_recipient))
            {
                ...
                maybe_notify_server_start(new_recipient,worker,typing_start_wait_period, typing_stop_wait_period);
                return;
            }
    ...
    return (
        <div className="absolute bottom-0 w-full mb-3 flex">
            ...
            onChange={(e)=>{
                ...
                update(worker,recipient,CHATAPP.typing_stop_wait_period,CHATAPP.typing_stop_wait_period);
            }}
            onKeyDown={(e)=>{
