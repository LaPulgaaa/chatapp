...
export default function Message({data}:{data:RecievedMessage}){
    ...
    return <div key={Math.floor(Math.random()*100000)} className={`flex m-4  p-2 rounded-full ${data.payload.message.user===name?'justify-end':data.payload.message.user==='mybot'?' justify-center':''}  `}>
